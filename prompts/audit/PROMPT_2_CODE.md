...
RET-12 Evaluated unit identity — RAG evidence names the full evaluated unit: corpus + query policy + retriever + router + reranker + context assembly + harness + delivery path + model/prompt + retry/stopping policy + permissions + citations + scorer + environment. Missing identity for release-significant evidence = P1.
RET-13 Lexical baseline / justified exception — text RAG includes grep/BM25/sparse baseline or a documented exception. Dense retrieval accepted without comparable lexical baseline = P1.
RET-14 Machine eval provenance and exact inputs — manifest, cases, observations, result, and comparison refs include repo-relative paths, SHA-256 hashes, condition/config fingerprint, dataset/corpus identity, and current commit policy. Hash mismatch, stale exact-HEAD empirical evidence, or unknown schema fields accepted silently = P0/P1.
RET-15 Harness and delivery-path evaluation — agentic RAG records harness type/version, delivery profile, retrieval-call count, files/artifacts opened/read, returned vs consumed results, retries, termination, tokens, latency, cost, and unsuccessful read/integration paths. Retriever-only agentic evidence = P1.
RET-16 Routed topology and fallback evaluation, conditional — routed/domain RAG has activation criteria, taxonomy/version, router/fallback policy, domain/collection match, route coverage, wrong-route, no-route/ambiguous behavior, cross-domain leakage, route latency/cost, drift checks, and a case where wrong route hides gold evidence. Missing for active routed RAG = P1; ACL/domain leakage = P0.
RET-17 Noise and hard-negative robustness — eval includes clean/noise/hard-distractor/duplicate slices when corpus noise is relevant and localizes failure stage: query formulation, retrieval miss, returned not opened, opened not used, premature stop, truncation, or generation failure. Production robustness claim without these slices = P1.
RET-18 Perturbation/attribution validity, conditional — graph/structured attribution records node deletion, edge deletion, synonym/paraphrase, dedup ablation, influential component ranking, perturbation budget/cost, and agreement with human or independent labels. Self-referential semantic shift as sole high-risk oracle = P0/P1.
RET-19 Online/drift and rollback signals for production claims — production RAG claims include no-answer rate, route/fallback distribution, retrieval/generation latency, corpus/index age, citation-open/acceptance or correction signals, quality/cost drift, rollback trigger, and re-eval trigger after corpus, embedding, router, reranker, prompt, model, judge, or policy changes. Missing for production claim = P1.
