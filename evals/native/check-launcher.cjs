...
(async()=>{
 ...
 assert.equal(await page.locator('#prompt-helper').getAttribute('open'),null);
 for(const width of [390,1280]){
  await page.setViewportSize({width,height:950});
  assert.equal(await page.evaluate(()=>document.documentElement.scrollWidth<=innerWidth+1),true);
  await page.screenshot({path:path.join(out,`start-${width}.png`),fullPage:true});
 }
 await page.locator('#prompt-helper > summary').click();
 ...
 assert.equal(await page.locator('#prompt-helper').evaluate(e=>e.open),true);
 ...
 const result={optional_helper:true,combinations,empty_input:'pass',scenario_prefills:'pass',clipboard:'pass',clipboard_fallback:'pass',first_run_example:'pass',install_and_help_prompts:'pass',archive_edition:archiveEdition,input_as_text:'pass',responsive:[390,1280],page_errors:errors,external_requests:external};
