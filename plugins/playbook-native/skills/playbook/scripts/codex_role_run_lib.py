...
def resolve_root(root: Path, *, allow_plain: bool = False) -> Path:
    resolved = root.expanduser().resolve()
    if not resolved.is_dir():
        raise RoleRunError(f"repository root does not exist: {resolved}")
    if not allow_plain and not (resolved / ".git").exists():
...
def native_snapshot(root: Path) -> dict[str, dict[str, Any]]:
    """Capture source files without requiring Git; never traverse dependency trees."""
    ignored = {'.git', '.playbook-artifacts', 'node_modules', '.venv', '__pycache__', '.pytest_cache'}
    snapshot = {}
...
def native_head(root: Path) -> str | None:
    if not (root / '.git').exists() or not shutil.which('git'):
        return None
    try:
        return current_head(root)
    except RoleRunError:
        return None  # An unborn checkout has no commit; do not invent one.


def native_prompt(root: Path, request: Path, role: str) -> str:
    request = resolve_inside(root, request, must_exist=True)
    brief = request.read_text(encoding='utf-8').strip()
    if not brief or len(brief.encode('utf-8')) > 48_000:
        raise RoleRunError('Native review request must contain 1–48000 UTF-8 bytes')
    return (
        f'You are the independent {role} reviewer for this project.\n'
        ...
        f'End with exactly one line: {ROLE_SPECS[role].marker}: PASS or ADVISORY or STOP_SHIP.\n'
        'Use PASS only when the requested scope has no confirmed blocker; disclose missing checks.\n\n'
        '<review_request>\n' + brief + '\n</review_request>\n'
    )


def run_native_process(argv: Sequence[str], *, root: Path, env: Mapping[str, str],
                       prompt: str, timeout: int) -> subprocess.CompletedProcess[str]:
    """Pass task text on stdin, including on Windows; stop the process group on timeout."""
    process = subprocess.Popen(list(argv), cwd=root, env=dict(env), text=True, encoding='utf-8',
...
def validate_trace(path: Path, *, require_completion: bool = False) -> int:
    if not path.is_file():
    ...
    if require_completion and (not completed or failed):
        raise RoleRunError('Codex review did not complete successfully')
    return count
...
def validate_role_result(
    *,
    ...
    root = resolve_root(root, allow_plain=True)
    ...
    native = payload.get('profile') == 'native'
    head = native_head(root) if native else current_head(root)
    if require_current_head and payload.get("base_commit") != head:
    ...
    if native and require_current_head:
        context = json.loads(resolve_inside(root, payload['inputs']['context_manifest_path'], must_exist=True).read_text(encoding='utf-8'))
        if context.get('workspace_snapshot') != native_snapshot(root):
            raise RoleRunError('Native review is stale: project files changed since review')
...
def execute_role_run(
    *,
    ...
    publish: bool,
    profile: str = 'governed',
    request: Path | None = None,
) -> tuple[dict[str, Any], Path]:
    native = profile == 'native'
    if profile not in {'native', 'governed'}:
        raise RoleRunError('Unknown execution profile')
    if native and os.environ.get('PLAYBOOK_REVIEW_WORKER') == '1':
        raise RoleRunError('A reviewer must not recursively start another reviewer')
    root = resolve_root(root, allow_plain=native)
    ensure_safe_id(task_id, label="task_id")
    if role not in ROLE_SPECS:
        raise RoleRunError(f"unsupported Codex role: {role}")
    spec = ROLE_SPECS[role]
    if not native and role in {"product_design_review", "program_design_review"} and not feature_id:
        raise RoleRunError(f"{role} requires --feature-id")
    if not native and role in {"slice_review", "maintainability_review"} and (not feature_id or not slice_id):
    ...
    if native and request is None:
        raise RoleRunError('Native review requires --request with the user goal and review scope')
    if native:
        publish = False  # Native consumes the immutable report; no governance registries.
    ...
    started_at = utc_now()
    base_commit = native_head(root) if native else current_head(root)
    ...
    before = native_snapshot(root) if native else workspace_snapshot(root, excluded_prefixes=excluded)
    prompt, renderer_argv = (native_prompt(root, request, role), ['native-request', str(request)]) if native else render_role_prompt(
        root=root,
        task_id=task_id,
        role=role,
        feature_id=feature_id,
    ...
    if native:
        context['workspace_snapshot'] = before
    ...
    if native:
        codex_argv.extend(['--ephemeral', '--skip-git-repo-check', '-c', 'approval_policy="never"', '-'])
    else:
    ...
    timed_out = False
    try:
        execution = (run_native_process(codex_argv, root=root, env=env, timeout=timeout_seconds, prompt=prompt)
                     if native else run_command(codex_argv, cwd=root, env=env, timeout=timeout_seconds))
    ...
    after = native_snapshot(root) if native else workspace_snapshot(root, excluded_prefixes=excluded)
    ...
    try:
        event_count = validate_trace(trace_path, require_completion=native)
        trace_valid = True
    ...
    if not report_path.is_file() or not report_path.read_text(encoding="utf-8").strip():
        ...
        try:
            if native:
                markers = re.findall(rf'(?m)^{spec.marker}: (PASS|ADVISORY|STOP_SHIP)\s*$', report_path.read_text(encoding='utf-8'))
                if len(markers) != 1:
                    raise RoleRunError('Native review needs exactly one verdict marker')
                parsed = {'verdict': markers[0]}
            else:
