...
def table(title: str, rows: dict[str, dict[str, float]], *, limit: int = 20) -> list[str]:
    lines = [
        f"## {title}",
        "",
        "| Key | Entries | Cost USD | Tokens | Retries | Tool Calls | Avg Latency ms |",
        "|-----|---------|----------|--------|---------|------------|----------------|",
    ]
    ...
    for key, values in sorted_rows[:limit]:
        ...
        lines.append(
            "| {key} | {entries} | {cost:.6f} | {tokens} | {retries} | {tools} | {latency:.1f} |".format(
                key=key,
                entries=entries,
    ...
    if not sorted_rows:
        lines.append("| none | 0 | 0.000000 | 0 | 0 | 0 | 0.0 |")
    lines.append("")
    return lines
...
def render_markdown(summary: dict[str, Any], findings: list[Finding]) -> str:
    ...
    lines = [
        ...
        "## Totals",
        "",
        "| Entries | Cost USD | Tokens | Retries | Tool Calls | Avg Latency ms |",
        "|---------|----------|--------|---------|------------|----------------|",
        "| {entries} | {cost:.6f} | {tokens} | {retries} | {tools} | {latency:.1f} |".format(
            entries=entries,
