...
actual execution contract: preflight policy, exact prompt materialization, fresh
...
def validate_role_result(
    *,
    ...
    if role not in ROLE_SPECS:
        raise RoleRunError(f"unsupported role in result: {role!r}")
    ...
    hash_fields = [
        ...
        (
            payload["inputs"]["context_manifest_path"],
            payload["inputs"]["context_manifest_sha256"],
        ),
    ...
    if payload["outputs"].get("report_path"):
        hash_fields.append(
            (payload["outputs"]["report_path"], payload["outputs"]["report_sha256"])
        )
    ...
    *,
    ...
    try:
        ...
        if isinstance(exc.stdout, bytes):
            execution_stdout = exc.stdout.decode("utf-8", errors="replace")
        else:
            execution_stdout = exc.stdout or ""
        if isinstance(exc.stderr, bytes):
            execution_stderr = exc.stderr.decode("utf-8", errors="replace")
        else:
    ...
    if not errors:
        ...
        if publish:
            ...
            review_record_path = write_design_review_record_if_supported(
                ...
                reviewer_binding=(
                    f"codex-role-run:{run_id}:{version}:{model or 'default'}"
                ),
            )
            append_event(
                ledger_path,
        ...
        status = "postflight_failed" if execution_exit == 0 else "execution_failed"
